...
function Info(props) {
  ...
  const {InitialText, InitialImage, setTemSize} = useData();
  const sample = [
  ...
  const onClickButton = async (id) => {
    ...
    if(Ed.data) {
      InitialText(Ed.data[2].map((el, i) => ({id: `${i + 1}`, x: el.x, y: el.y, content: el.content, size: el.size, font: String(el.font), bold: el.bold, italic: el.italic, underlined: el.underlined, align: el.align, textcolor: el.textcolor, textopa: el.textopa, backcolor: el.backcolor, backopa: el.backopa, zindex: el.zindex, flag: true})))
      InitialImage(Ed.data[0].map((el, i) => ({id: `${i + 1}`, x: el.x, y: el.y, src: el.src, width: el.width, height: el.height, zindex: el.zindex, borderstyle: el.borderstyle, bordersize: el.bordersize, bordercolor: el.bordercolor, opacity: el.opacity, radius: el.radius, blur: el.blur, brightness : el.brightness, contrast: el.contrast, grayscale: el.grayscale, hue: el.hue, invert:el.invert, saturate: el.saturate, sepia: el.sepia})))     
      setTemSize(Ed.data[1].width, Ed.data[1].height)
      navigate('/edit')
    }
  };
