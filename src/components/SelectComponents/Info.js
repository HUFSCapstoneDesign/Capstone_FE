...
function Info(props) {
  ...
  const onClickButton = async (id) => {
    ...
    if(Ed.data) {
      ...
      const Idata = Ed.data[0].map((el, index) => ({id: `${index + 1}`, x: el.x, y: el.y, src: el.src, width: el.width, height: el.height, zindex: el.zindex, borderstyle: el.boardstyle, bordersize: el.bordersize, bordercolor: el.bordercolor, opacity: el.opacity, radius: el.radis, blur: el.blur, brightness : el.brightness, contrast: el.contrast, grayscale: el.grayscale, hue: el.hue, invert:el.invert, saturate: el.saturate, sepia: el.sepia}))
